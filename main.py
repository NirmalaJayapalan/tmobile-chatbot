...
def find_devices(query):
    ...
    for d in telecom_data["devices"]:
        ...
        if "budget" in query_lower or "low" in query_lower:
            if d.get("budget","") != "low": continue
        if "medium" in query_lower and d.get("budget","") != "medium": continue
        if "high" in query_lower and d.get("budget","") != "high": continue
        results.append(d)
    return results

deffrom flask import Flask, render_template, request, jsonify
...
def find_plans(query):
    ...
    for p in telecom_data["plans"]:
        if any(word in p["name"].lower() for word in query_lower.split()): results.append(p)
    return results
...
def find_promotions(query):
    ...
    for promo in telecom_data["promotions"]:
        if any(word in promo["description"].lower() for word in query_lower.split()): results.append(promo)
    return results
...
def ask():
    ...
    return jsonify({"devices": devices, "plans": plans, "promotions": promotions})
